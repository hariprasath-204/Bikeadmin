document.addEventListener("DOMContentLoaded", () => {
    ...
    const loadBikes = async () => {
        ...
        tbody.innerHTML = bikes.map(b => `
            <tr>
                ...
                <td>${b.engine || ""}</td>
                <td>${b.mileage || ""}</td>
                <!-- Corrected path to use the root 'images' folder -->
    ...
    // ... (rest of the booking tables and contacts code remains the same)
    ...
    const loadAndRenderBookings = async (config) => {
        ...
        const bodies = {};
        config.statuses.forEach(status => { bodies[status] = document.getElementById(config.tbodyIds[status]); });
        Object.values(bodies).forEach(body => { if (body) body.innerHTML = ""; });
        data.forEach(item => {
            const rowHtml = config.renderRow(item);
            if (bodies[item.status]) { bodies[item.status].innerHTML += rowHtml; }
        });
    };
    const bookingConfigs = {
        testdrive: {
            endpoint: "testdrive-bookings", statuses: ["pending", "confirmed", "completed", "cancelled"],
            tbodyIds: { pending: "testdriveTableBodyPending", confirmed: "testdriveTableBodyConfirmed", completed: "testdriveTableBodyCompleted", cancelled: "testdriveTableBodyCancelled" },
            renderRow: (b) => `<tr><td>${b.booking_id}</td><td>${b.full_name}</td><td>${b.mobile}</td><td>${b.email}</td><td>${b.bike_model}</td><td>${new Date(b.preferred_date).toLocaleDateString()}</td><td>${b.preferred_time}</td><td>${createStatusDropdown(b.booking_id, b.status, "testdrive")}</td></tr>`
        },
        service: {
            endpoint: "service-bookings", statuses: ["pending", "confirmed", "completed", "cancelled"],
            tbodyIds: { pending: "serviceBookingsTableBodyPending", confirmed: "serviceBookingsTableBodyConfirmed", completed: "serviceBookingsTableBodyCompleted", cancelled: "serviceBookingsTableBodyCancelled" },
            renderRow: (b) => `<tr><td>${b.booking_id}</td><td>${b.full_name}</td><td>${b.bike_model}</td><td>${b.service_type}</td><td>${new Date(b.preferred_date).toLocaleDateString()}</td><td>${createStatusDropdown(b.booking_id, b.status, "service")}</td></tr>`
        },
        bike: {
            endpoint: "bike-bookings", statuses: ["pending", "confirmed", "completed", "cancelled"],
            tbodyIds: { pending: "bookingsTableBodyPending", confirmed: "bookingsTableBodyConfirmed", completed: "bookingsTableBodyCompleted", cancelled: "bookingsTableBodyCancelled" },
            renderRow: (b) => `<tr><td>${b.booking_id}</td><td>${b.bike_name}</td><td>${b.user_id || "Guest"}</td><td>${b.full_name}</td><td>${b.mobile}</td><td>${b.email}</td><td>${new Date(b.created_at).toLocaleString()}</td><td>${createStatusDropdown(b.booking_id, b.status, "bike")}</td></tr>`
        }
    };
    ...
    document.body.addEventListener("click", async (e) => {
        ...
        if (action === "edit-bike") {
            ...
            if (bike.thumbnail) {
                // Corrected path for edit preview
